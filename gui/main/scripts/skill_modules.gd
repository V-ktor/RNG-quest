...
func update():
	...
	for k in skill_module_dict.keys():
		...
		for i in range(skill_module_dict[k].size()):
			...
			check_button.set_pressed_no_signal(skill_module_dict[k][i] not in character_settings.disabled_skill_modules)
			...
			panel.show()
		(base_container.get_node(n) as Control).custom_minimum_size.y = maxi(int(container.size.y + 16), 24 + 88 * skill_module_dict[k].size())
